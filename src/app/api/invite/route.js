...
export async function POST(request){
  try{const body=await request.json(),ref=adminDb.collection('accountInvites').doc(body.id),snap=await ref.get();checked(snap.data(),body.id,body.secret);const data=snap.data();if(body.action==='preview')return Response.json({role:data.role,displayName:data.displayName,expiresAt:data.expiresAt.toDate().toISOString(),childCount:(data.childKeys||[]).length});
    const authorization=request.headers.get('authorization')||'';if(!authorization.startsWith('Bearer '))return Response.json({error:'アカウント登録後に招待を確定してください。'},{status:401});const user=await adminAuth.verifyIdToken(authorization.slice(7));if(!user.email)throw new Error('メールアドレスを確認できません。');

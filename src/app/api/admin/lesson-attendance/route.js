...
export async function POST(request) {
  try {
    ...
    const result = await adminDb.runTransaction(async (transaction) => {
      ...
      if(isMiddle&&action==='delete'&&appliedRewards.length){const data=userSnap.data(),nextExp=applyExperienceDelta(data,-rewardPointTotal),oldWordScore=Number(wordReward?.data().scoreCorrect??wordReward?.data().correct??0);transaction.update(userRef,{points:Number(data.points||0)+pointDelta,...(termId===currentTermId?{termPoints:Number(data.termPoints||0)+pointDelta}:{}),totalEarnedPoints:Math.max(0,Number(data.totalEarnedPoints||0)-rewardEarnedTotal),experience:nextExp.experience,level:nextExp.level,homeworkCount:Math.max(0,Number(data.homeworkCount||0)-rewardHomeworkCount),wordTestCount:Math.max(0,Number(data.wordTestCount||0)-Number(Boolean(wordReward?.data().completed))),totalWordTestScore:Math.max(0,Number(data.totalWordTestScore||0)-oldWordScore),...(termId===currentTermId?{termHomeworkCount:Math.max(0,Number(data.termHomeworkCount||0)-rewardHomeworkCount),termWordTestCount:Math.max(0,Number(data.termWordTestCount||0)-Number(Boolean(wordReward?.data().completed))),termWordScore:Math.max(0,Number(data.termWordScore||0)-oldWordScore)}:{}),lastUpdated:now});}
      if (isHigh) {

...
export async function GET(request) {
  try {
    ...
    const [shared, sharedYear,profile,courseSessionDocs,courseApplicationDocs,interviewReservationDocs,courseProgramDocs,interviewPeriodDocs] = await Promise.all([
      readSharedPortal(),
      readSharedYear(year),
      adminDb.collection('studentProfiles').doc(studentKey).get(),
      adminDb.collection('courseLessonPlans').where('studentKey','==',studentKey).get(),
      adminDb.collection('courseApplications').where('studentKey','==',studentKey).get(),
      adminDb.collection('interviewReservations').where('studentKey','==',studentKey).get(),
    ...
    const shiftWeeks=await adminDb.collection('weeklyShifts').where('weekStart','>=',shiftWeekStart(yearStart)).where('weekStart','<=',shiftWeekStart(yearEnd)).get();
    const confirmedDates=new Set(),shiftedRegular=[],shiftedMakeups=[];
    shiftWeeks.docs.filter(doc=>doc.data().status==='confirmed').forEach(doc=>{for(let index=0;index<6;index++){const day=new Date(`${doc.id}T12:00:00+09:00`);day.setUTCDate(day.getUTCDate()+index);confirmedDates.add(day.toISOString().slice(0,10));}for(const entry of doc.data().entries||[]){if(entry.studentKey!==studentKey||entry.date<yearStart||entry.date>yearEnd)continue;const period=SHIFT_PERIODS.find(item=>item.id===entry.periodId);if(entry.lessonType==='regular')shiftedRegular.push({id:`regular_shift_${entry.id}`,name:'通常授業',type:'lesson',startDate:entry.date,endDate:entry.date,startTime:period?.startTime||'',endTime:period?.endTime||'',detail:entry.subject||'',source:'weeklyShifts'});if(entry.lessonType==='makeup')shiftedMakeups.push({id:`makeup_shift_${entry.id}`,name:'振替授業',type:'makeup',startDate:entry.date,endDate:entry.date,startTime:period?.startTime||'',endTime:period?.endTime||'',source:'weeklyShifts'});}});
    const regularEvents=[...regular.filter(item=>!confirmedDates.has(item.startDate)),...shiftedRegular];
    const middlePromises=full&&grade>=7&&grade<=9?[1,2,3].map(term=>adminDb.collection(elementary?'adminStudents':'users').doc(student.id).collection('lessonTerms').doc(`${year}_${term}`).collection('records').get()):[];
    ...
    const applicationStatus=new Map(courseApplicationDocs.docs.map(doc=>[doc.data().programId,doc.data().status])),reservedPeriodIds=new Set(interviewReservationDocs.docs.filter(doc=>['pending','confirmed'].includes(doc.data().status)).map(doc=>doc.data().periodId)),actionItems=[...courseProgramDocs.docs.filter(doc=>{const item=doc.data(),status=applicationStatus.get(doc.id);return item.application?.enabled===true&&(!item.application.deadline||item.application.deadline>=today)&&(!status||status==='cancelled')}).map(doc=>({id:`course_${doc.id}`,type:'course',title:`${doc.data().name||'講習'}の申込`,deadline:doc.data().application?.deadline||'',message:'講習の申込内容をご確認ください。'})),...interviewPeriodDocs.docs.filter(doc=>{const item=doc.data();return item.active!==false&&item.endDate>=today&&(!item.deadline||item.deadline>=today)&&!reservedPeriodIds.has(doc.id)}).map(doc=>({id:`interview_${doc.id}`,type:'interview',title:`${doc.data().name||'保護者面談'}の予約`,deadline:doc.data().deadline||doc.data().endDate||'',message:'面談日時を選択してください。'}))].sort((a,b)=>(a.deadline||'9999').localeCompare(b.deadline||'9999')),courses=courseSessionDocs.docs.filter(doc=>doc.data().status==='confirmed').map(doc=>({id:`course_${doc.id}`,name:'講習授業',type:'course',startDate:doc.data().date,endDate:doc.data().date,startTime:doc.data().startTime||'',endTime:doc.data().endTime||'',detail:doc.data().subject||'',source:'courseLessonPlans'})).filter(item=>item.startDate>=yearStart&&item.startDate<=yearEnd),interviews=interviewReservationDocs.docs.filter(doc=>doc.data().status==='confirmed').map(doc=>({id:`interview_${doc.id}`,name:'保護者面談',type:'interview',startDate:doc.data().date,endDate:doc.data().date,startTime:doc.data().startTime||'',endTime:doc.data().endTime||'',source:'interviewReservations'})).filter(item=>item.startDate>=yearStart&&item.startDate<=yearEnd);
    ...
    return Response.json({ year, detail:full?'full':'summary', actionItems, announcements: active(cleanItems(merge(data.announcements,announcementDocs))).slice(0,100), documents: active(cleanItems(merge(data.documents,documentDocs), true)).slice(0,100), teachingDates, calendarEvents:[...regularEvents,...makeups.filter(item=>!shiftedMakeups.some(shift=>shift.startDate===item.startDate)),...shiftedMakeups,...courses,...interviews,...manual].sort((a,b)=>a.startDate.localeCompare(b.startDate)||a.name.localeCompare(b.name,'ja')) });

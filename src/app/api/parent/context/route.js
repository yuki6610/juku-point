...
export async function GET(request) {
  try {
    const parent = await requireParent(request),project=(key,data,adminTest=false)=>({key,name:data.realName||data.name||data.displayName||(adminTest?'管理者':'名前未設定'),grade:Number(data.grade||0),weekdays:(data.lessonSchedule?.weekdays||data.weekdays||[]).map(Number),courseTags:Array.isArray(data.courseTags)?data.courseTags:[],adminTest});
    let children=[];

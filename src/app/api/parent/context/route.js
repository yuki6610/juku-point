...
export async function GET(request) {
  try {
    ...
    try {
      ...
      const ids=parent.role==='admin'?[]:children.filter(child=>child.grade>=7&&child.grade<=9).map(child=>child.key);
      ...
      const term=`${currentTermId.split('_')[1]}学期`,snapshots=await Promise.all(ids.map(async key=>{const elementary=key.startsWith('elementary_'),id=key.slice(elementary?11:5);return Promise.all([adminDb.collection('scoreSubmissionTerms').doc(currentTermId).collection('students').doc(elementary?key:id).get(),adminDb.collection('studentProfiles').doc(key).get(),adminDb.collection(elementary?'adminStudents':'users').doc(id).collection('scores').where('year','==',year).where('term','==',term).get()])}));
      const schools=[...new Set(entries.map(item=>String(item.schoolName||'')).filter(Boolean))];
      submissionStatus=Object.fromEntries(snapshots.map(([status,profile,scores],index)=>{const child=children.find(item=>item.key===ids[index]),tagSchool=(child?.tags||[]).find(tag=>schools.some(name=>String(name).normalize('NFKC').replace(/\s+/g,'')===String(tag).normalize('NFKC').replace(/\s+/g,''))),matched=matchingSubmissionEntries(entries,{grade:child?.grade,schoolName:tagSchool||profile.data()?.schoolName||''},currentTermId);return [ids[index].replace(/^(user|elementary)_/,''),projectSubmissionStatus(matched,status.data()||{},japanDateId(),scores.docs.map(doc=>doc.data()))]}));
    } catch {}

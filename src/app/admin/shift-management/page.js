...
import TeacherPreferences from "../teacher-preferences/page";
...
export default function ShiftManagementPage(){
  ...
  return <main className="admin-hub-page"><header className="admin-hub-heading"><span>授業の準備</span><h1>シフト管理</h1><p>週のシフトを作成し、講師の希望を確認します。</p></header><AdminHubTabs label="シフト管理メニュー" tabs={tabs} active={tab} onChange={select}/><div className="admin-hub-content">{tab==="week"?<Shifts selectedWeek={week} onWeekChange={changeWeek}/>:<TeacherPreferences selectedWeek={week} onWeekChange={changeWeek}/>}</div></main>;
}

...
const tabs = [{ id:"requests",label:"登録申請" },{ id:"teachers",label:"講師" },{ id:"parents",label:"保護者" },{ id:"students",label:"生徒本人・進級" },{ id:"recovery",label:"ログイン復旧" },{ id:"admins",label:"管理者招待" }];
export default function AccountsPage(){
  ...
  return <main className="admin-hub-page"><header className="admin-hub-heading"><span>管理設定</span><h1>アカウント管理</h1><p>招待、子どもの紐付け、進級、ログイン復旧を行います。</p></header><AdminHubTabs label="アカウント管理メニュー" tabs={tabs} active={tab} onChange={select}/><div className="admin-hub-content">
    {tab==="requests"&&<AccountRequests/>}{tab==="teachers"&&<TeacherManager/>}{tab==="parents"&&<ParentManager/>}{tab==="students"&&<StudentAccounts/>}{tab==="recovery"&&<AccountRecovery/>}{tab==="admins"&&<AdminInviteManager/>}
  </div></main>;
}

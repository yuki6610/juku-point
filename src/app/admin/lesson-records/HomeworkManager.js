...
export default function HomeworkManager({ onDirtyChange, onBusyChange }) {
  ...
  return <section className="homework-panel"><h2>次回の宿題を登録</h2><p>複数の課題を1セットにまとめます。提出結果はここでは入力しません。</p>
    ...
    {data && student && <><fieldset disabled={busy} onChange={() => onDirtyChange(true)}><legend>{editing ? '宿題内容を変更' : '宿題セット'}</legend><label>指示日<input type="date" value={assignedDate} onChange={event => setAssignedDate(event.target.value)} /></label><label>確認予定日<input type="date" value={dueDate} min={assignedDate} onChange={event => setDueDate(event.target.value)} /></label>
      {items.map((item, index) => <div className="homework-row" key={index}><select aria-label={`教材${index + 1}`} value={item.materialId} onChange={event => setItems(items.map((row, n) => n === index ? { ...row, materialId: event.target.value } : row))}><option value="">教材を選択</option>{data.templates.materials.map(material => <option key={material.id} value={material.id}>{material.label}</option>)}</select><input aria-label={`範囲${index + 1}`} placeholder="例：P.20〜23／単語1〜20" value={item.range} onChange={event => setItems(items.map((row, n) => n === index ? { ...row, range: event.target.value } : row))} /><button disabled={items.length === 1} onClick={() => { setItems(items.filter((_, n) => n !== index)); onDirtyChange(true); }}>項目を外す</button></div>)}
      <button disabled={items.length >= 20} onClick={() => { setItems([...items, { materialId: '', range: '' }]); onDirtyChange(true); }}>＋宿題を追加</button><button onClick={save}>宿題セットを保存</button>{editing && <button onClick={reset}>編集を終了</button>}

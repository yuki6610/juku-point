...
export default function AdminRewardHistory(){
  ...
  return <div className="admin-history-container"><h1 className="admin-history-title">交換履歴管理</h1><p className="history-range-note">必要な生徒だけを読み込み、Firestoreの使用量を抑えます。</p>
    <label className="history-student-select">生徒<select value={selected} onChange={event=>choose(event.target.value)}><option value="">生徒を選択してください</option>{students.map(item=><option key={item.id} value={item.id}>{item.realName||item.displayName||'名前未設定'}</option>)}</select></label>

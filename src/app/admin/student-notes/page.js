...
export default function StudentNotesPage(){
  ...
  const load=async()=>{setNotice('');try{const response=await fetch('/api/admin/student-notes',{headers:{Authorization:`Bearer ${await token()}`}}),data=await response.json();if(!response.ok)throw new Error(data.error);setRows(data.rows||[])}catch(error){setNotice(error.message)}};
  useEffect(()=>{load()},[]);
  ...
  const save=async row=>{setSaving(row.key);setNotice('');try{const response=await fetch('/api/admin/student-notes',{method:'POST',headers:{'Content-Type':'application/json',Authorization:`Bearer ${await token()}`},body:JSON.stringify(row)}),data=await response.json();if(!response.ok)throw new Error(data.error);setNotice(`${row.name}さんのメモを保存しました。`)}catch(error){setNotice(error.message)}finally{setSaving('')}};

...
export default function CourseLessonsPage() {
  ...
  useEffect(() => {
    ...
    let active=true;setAssignments([]);
    adminApi(`?programId=${encodeURIComponent(programId)}`)
      .then((data) => {if(active)setAssignments(data.assignments || [])})
  ...
  const saveResult = async () => {
    ...
    setSaving(true);
    try {
      await adminApi("", { method: "PATCH", body: JSON.stringify({ programId, assignmentId: selected.id, checkedDate, result: selected.type === "homework"

...
export default function ParentPage() {
  const [data,setData]=useState(null),[student,setStudent]=useState(''),[tab,setTab]=useState('overview'),[lessons,setLessons]=useState(null),[report,setReport]=useState(null),[portal,setPortal]=useState(null),[term,setTerm]=useState(''),[error,setError]=useState(''),[busy,setBusy]=useState(false);
  useEffect(() => onAuthStateChanged(auth, async user => { if (!user) return location.href='/parent/login'; try { const value=await parentApi('/api/parent/context'); setData(value); setStudent(value.children[0]?.key || ''); parentApi('/api/parent/portal').then(setPortal).catch(error=>setError(error.message)); } catch(error) { setError(error.message); } }), []);
  useEffect(() => {
  ...
  return <main className="parent-shell">
    ...
    {data&&<>{data.adminPreview&&<p className="parent-alert">管理者プレビューです。小学生・中学生の保護者表示を確認できます。</p>}{data.children.length===0?<section><p>紐付けられた生徒がいません。教室へお問い合わせください。</p></section>:<>
      ...
      <nav className="parent-tabs"><button className={tab==='overview'?'active':''} onClick={()=>setTab('overview')}>概要</button><button className={tab==='homework'?'active':''} onClick={()=>setTab('homework')}>宿題</button><button className={tab==='lessons'?'active':''} onClick={()=>setTab('lessons')}>授業記録</button><button className={tab==='report'?'active':''} onClick={()=>setTab('report')}>学期レポート</button><button className={tab==='calendar'?'active':''} onClick={()=>setTab('calendar')}>カレンダー</button><button className={tab==='news'?'active':''} onClick={()=>setTab('news')}>お知らせ{tab!=='news'&&portal&&unreadCount(portal)>0?<b>{unreadCount(portal)}</b>:''}</button></nav>
      {tab==='overview'&&<ParentOverview child={child} lessons={lessons} portal={portal} busy={busy} go={setTab}/>}
      ...
      {tab==='calendar'&&<ParentCalendar portal={portal}/>}
      {tab==='news'&&<ParentNews portal={portal}/>}
    </>}</>}
  </main>;
}

const homeworkStatus = { submitted:'すべて提出', partial:'一部未完了', missed:'未提出', absent:'欠席・未確認', laterCompleted:'後日完了' };
const readKey='parent-portal-read';
...
function ParentOverview({child,lessons,portal,busy,go}) {
  ...
  return <section className="parent-overview"><div className="parent-section-heading"><div><small>OVERVIEW</small><h2>{child?.name}さんの学習状況</h2></div><span>最新情報</span></div>{busy&&!lessons?<p>読み込み中…</p>:<><div className="parent-overview-grid"><article><span>確認待ちの宿題</span><strong>{pending.length}<small>件</small></strong><button onClick={()=>go('homework')}>宿題を確認</button></article><article><span>最新の授業記録</span><strong className="overview-date">{latest?.date?.replaceAll('-',' / ')||'記録なし'}</strong><button onClick={()=>go('lessons')}>授業記録を見る</button></article><article><span>新しいお知らせ・資料</span><strong>{portal?unreadCount(portal):0}<small>件</small></strong><button onClick={()=>go('news')}>お知らせを見る</button></article></div><div className="parent-guide"><strong>このページで確認できること</strong><p>宿題、提出状況、授業中の様子、学期ごとの成績、営業日、お知らせと配布資料を確認できます。</p></div></>}</section>;
}

function ParentCalendar({portal}) { const dates=portal?.teachingDates||[], months=[...new Set(dates.map(date=>date.slice(0,7)))]; return <section><div className="parent-section-heading"><div><small>CLASS CALENDAR</small><h2>{portal?.year}年度 営業日カレンダー</h2></div><span>色付きの日が授業日</span></div><div className="parent-calendar">{months.map(month=><CalendarMonth key={month} month={month} teaching={new Set(dates)}/>)}</div>{!dates.length&&<p>営業日はまだ公開されていません。</p>}</section>; }
...
function ParentNews({portal}) { useEffect(()=>{if(!portal)return;localStorage.setItem(readKey,JSON.stringify([...(portal.announcements||[]),...(portal.documents||[])].map(item=>item.id)))},[portal]); return <section><div className="parent-section-heading"><div><small>NEWS & DOCUMENTS</small><h2>教室からのお知らせ</h2></div></div><div className="parent-news"><h3>お知らせ</h3>{!(portal?.announcements||[]).length?<p>現在のお知らせはありません。</p>:(portal.announcements||[]).map(item=><article key={item.id}><time>{item.date}</time><strong>{item.title}</strong><p>{item.body}</p></article>)}<h3>PDF資料</h3>{!(portal?.documents||[]).length?<p>公開中の資料はありません。</p>:(portal.documents||[]).map(item=><a key={item.id} href={item.url} target="_blank" rel="noopener noreferrer"><span><time>{item.date}</time><strong>{item.title}</strong><small>{item.body}</small></span><b>PDFを開く ↗</b></a>)}</div></section>; }

...
export default function TeacherPage() {
  ...
  const [reviewId, setReviewId] = useState(''), [reviewStatus, setReviewStatus] = useState('completed'), [commentIds, setCommentIds] = useState([]), [late, setLate] = useState(false), [forgot, setForgot] = useState(false), [note, setNote] = useState('');
  ...
  const save = async () => {
    ...
    try {
      ...
      const homeworkReview = reviewId ? { assignmentId: reviewId, status: reviewStatus, missingIds: [] } : null;
      const learningRecord = { homework: reviewId ? (reviewStatus === 'completed' ? 'submitted' : reviewStatus === 'partial' ? 'partial' : 'missed') : 'none', wordTest: { status: 'notScheduled', correct: null, total: null }, late, forgot, behaviorNote: note };
      ...
      setNotice('授業後の記録を保存しました。'); setReviewId(''); setCommentIds([]); setLate(false); setForgot(false); setNote(''); setDueDate(''); setNextItems([{ materialId: '', range: '' }]);
  ...
  return <main className="teacher-shell"><header><div><small>TEACHER CONSOLE</small><h1>授業後の入力</h1><p>{context?.displayName || ''}　その日に担当した生徒を選択してください。</p></div><button onClick={() => signOut(auth).then(() => location.href='/teacher/login')}>ログアウト</button></header>
    ...
    {student && homeworkData && <><section><h2>今回の確認</h2>{pending.length ? <><label>確認する宿題<select value={reviewId} onChange={e => setReviewId(e.target.value)}><option value="">今回は確認しない</option>{pending.map(item => <option key={item.id} value={item.id}>{item.dueDate}予定：{item.items.map(row => `${row.materialLabel} ${row.range}`).join('／')}</option>)}</select></label>{reviewId && <label>実施状況<select value={reviewStatus} onChange={e => setReviewStatus(e.target.value)}><option value="completed">すべて実施</option><option value="partial">一部未実施</option><option value="missed">未実施</option></select></label>}</> : <p>確認待ちの宿題はありません。</p>}

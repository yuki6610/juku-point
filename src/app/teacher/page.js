...
export default function TeacherPage() {
  ...
  const save = async () => {
    ...
    try {
      ...
      setNotice('授業後の記録を保存しました。'); setReviewId(''); setItemResults({}); setCommentIds([]);setWordCorrect('');setWordTotal('');setAttendance('present');setOriginalDate(''); setLate(false); setForgot(false); setNote(''); setDueDate(''); setNextItems([{ materialId: '', range: '' }]);
  ...
  return <main className="teacher-shell"><header><div><small>TEACHER CONSOLE</small><h1>授業後の入力</h1><p>{context?.displayName || ''}　その日に担当した生徒を選択してください。</p></div><button onClick={() => signOut(auth).then(() => location.href='/teacher/login')}>ログアウト</button></header>
    {notice && <p className="teacher-notice" role="status">{notice}</p>}<section className="teacher-flow"><label><span>1. 授業日</span><input type="date" value={date} onChange={e => setDate(e.target.value)} /></label><label><span>2. 担当生徒</span><select value={studentKey} onChange={e => setStudentKey(e.target.value)}><option value="">選択してください</option>{(context?.students || []).map(item => <option key={item.key} value={item.key}>{item.name}（{gradeLabel(item.grade)}）</option>)}</select></label></section>{student&&<section><h2>出席状況</h2><div className="teacher-attendance">{[['present','出席'],['absent','欠席'],['makeup','振替出席']].map(([value,label])=><button type="button" key={value} className={attendance===value?'selected':''} onClick={()=>setAttendance(value)}>{label}</button>)}</div>{attendance==='makeup'&&<label>振替元の授業日<input type="date" value={originalDate} onChange={e=>setOriginalDate(e.target.value)}/></label>}</section>}

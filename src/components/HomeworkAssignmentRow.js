...
export default function HomeworkAssignmentRow({ item, materials, elementary=false, onChange, onRemove, removeDisabled=false }) {
  ...
  return <div className="homework-assignment-row">
    ...
    <input aria-label={material?.rangeType==='number'?'番号範囲':'ページ・範囲・内容'} inputMode="text" placeholder={material?.rangeType==='number'?'例：101-150':material?.customLabel?'ページがない場合は空欄':'例：10-15、漢字練習'} value={item.range||''} onChange={event=>patch({range:event.target.value})}/>

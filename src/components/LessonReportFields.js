...
export default function LessonReportFields({ learningContent, onLearningContentChange, value, onChange, context = {}, studentKey = '', lessonDate = '', teacherView=false }) {
  ...
  useEffect(()=>{if(!facts.subject&&context.subject)onChange({...facts,subject:context.subject})},[context.subject]);
  ...
  return <section className="lesson-report-fields">
    ...
    <label className="lesson-report-main">授業報告（保護者に送信する文章）<textarea rows="10" value={facts.extraNote||''} maxLength={2000} onChange={event=>onChange({...facts,extraNote:event.target.value})} placeholder="「授業報告を生成」を押すと文章が入ります。内容を確認し、必要に応じて修正してください。" /></label>

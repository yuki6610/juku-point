...
export default function RoleLogin({ mode }) {
  ...
  const submit=async event=>{
    ...
    try {
      const auth=getAuth();await auth.authStateReady?.();const credential=await signInWithEmailAndPassword(auth,email,password), user=credential.user, token=await user.getIdToken();

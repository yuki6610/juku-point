...
export default function FinalCorrelationChart({ points, meUid }) {
  ...
  const safePoints = points.filter(p => {
    const behavior = Number(p.behavior)
    const score = Number(p.score)

    // 数値チェック
    if (!Number.isFinite(behavior)) return false
    if (!Number.isFinite(score)) return false

    // 範囲チェック
    if (behavior < 0 || behavior > 100) return false
    if (score < 0 || score > yMax) return false

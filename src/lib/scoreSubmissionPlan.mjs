...
export function projectSubmissionStatus(entries, saved = {}, today = new Date().toISOString().slice(0, 10)) {
  const items = entries.map(entry => {
    ...
    const status = explicit?.received === true ? 'received' : explicit?.received === false ? 'missing'
  ...
  const received = kind => { const due=forKind(kind).filter(item=>item.status!=='upcoming'); return due.length ? due.every(item => ['received', 'legacy'].includes(item.status)) : true; };

export const DEFAULT_HOMEWORK_TEMPLATES = {
  materials: [{ id: 'elementary_text', label: '小学生教材', audience: 'elementary', subject:'all' }, { id: 'new_english', label: '新ワーク 英語', audience: 'middle', subject:'english' }, { id: 'new_math', label: '新ワーク 数学', audience: 'middle', subject:'math' }, { id: 'summing', label: 'サミングアップ', audience: 'middle', subject:'all' }, { id: 'words', label: '単語テスト', audience: 'middle', subject:'english', rangeType: 'number' }, { id: 'other', label: 'その他の宿題', audience: 'all', subject:'all' }],
...
export function validateTemplates(value) {
  ...
  return { materials: value.materials.map(({ id, label, audience, subject, rangeType }) => ({ id, label: label.trim(), audience: ['elementary','middle','all'].includes(audience) ? audience : 'all', subject: ['japanese','math','english','science','social','all'].includes(subject) ? subject : 'all', ...(rangeType === 'number' ? { rangeType } : {}) })), comments: value.comments.map(({ id, label, tone }) => ({ id, label: label.trim(), tone: tone === 'negative' ? 'negative' : 'positive' })), results: Object.fromEntries(Object.keys(RESULT_LABELS).map(key => [key, value.results[key].trim()])) };
}
export function validateAssignment(input, templates) {
  ...
  const items = input.items.map((item, index) => {
    ...
    return { id: String(index), materialId: material.id, materialLabel: material.label, subject: material.subject || 'all', ...(material.rangeType ? { rangeType:material.rangeType } : {}), range: item.range.trim() };
  });
...
export function publicAssignment(data) {
  ...
  return { assignedDate: data.assignedDate, dueDate: data.dueDate, items: (data.items || []).map(item => ({ id: item.id, materialId: item.materialId, materialLabel: item.materialLabel, ...(item.subject ? { subject:item.subject } : {}), ...(item.rangeType ? { rangeType:item.rangeType } : {}), range: item.range })), review: result(data.review), laterCompletion: result(data.laterCompletion) };
}

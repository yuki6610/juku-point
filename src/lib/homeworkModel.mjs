...
export const DEFAULT_HOMEWORK_TEMPLATES = {
  materials: [{ id: 'elementary_text', label: '小学生教材', audience: 'elementary', subjects:['japanese','arithmetic'], difficulty:2 }, { id: 'new_english', label: '新ワーク 英語', audience: 'middle', subjects:['english'], difficulty:2 }, { id: 'new_math', label: '新ワーク 数学', audience: 'middle', subjects:['math'], difficulty:2 }, { id: 'summing', label: 'サミングアップ', audience: 'middle', subjects:['japanese','math','english','science','social'], difficulty:5 }, { id: 'words', label: '単語テスト', audience: 'middle', subjects:['english'], difficulty:2, rangeType: 'number' }, { id: 'other', label: 'その他の宿題', audience: 'all', subjects:['all'], difficulty:3, customLabel:true }],
...
export function validateAssignment(input, templates) {
  ...
  const items = input.items.map((item, index) => {
    ...
    return { id: String(index), materialId: material.id, materialLabel: customLabel || material.label, templateLabel:material.label, subject:selectedSubject || 'all', subjects:materialSubjects(material), difficulty, ...(customLabel?{customLabel}:{}), ...(material.rangeType ? { rangeType:material.rangeType } : {}), range };
  });

...
test('invitation identifiers and roles are restricted',()=>{
  assert.equal(validInviteId('a'.repeat(32)),true);assert.equal(validInviteId('../admin'),false);assert.equal(validInviteRole('teacher'),true);assert.equal(validInviteRole('parent'),true);assert.equal(validInviteRole('admin'),true);assert.equal(validInviteRole('student'),false);
});
